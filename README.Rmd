...
Existe uma tendência de aumento monotônica mundial da concetração de CO2 na atmosfera, assim, ela deve ser retirar para podermos observar as tendências gerionais.
...
Plot da concentração de CO2 no ano de 2014, primeiro ano de utilização do satélite.
...
A partir da função point.in.pol do pacote sp, criamos afunção abaixo para facilitar o processo de filtragem em função de um polígono definido.
...
Obseve que houve uma falha na região nordeste, podemos então pedir os pontos presentes do polígono br ou nordeste.
...
Agora podemos filtrar os pontos para o território brasileiro e salvar o arquivo na pasta **data/oco2_br.rds**.
...
oco2_br %>% 
  ...
  ggplot2::theme_bw()
```
A região Norte apresenta uma menor sazonalidade com as menores variações da concnetração de CO2 atmosférico do que aquela comparada às demais regiões.
...
De posse desse mapa, vamos fazer a análise de dependência espacial. Definindo os valores de X e Y e a fórmula para análise da autocorrelação espacial.
```{r}
...
## Motivação Final
### Criar para cada região o mapa de interpolação via krigagem ordinária por estado ao longo por trimestre por ano. Os modelos testados serão Esférico, Gaussiano e Exponencial para modelagem variográfica
...
geoestati <- function(df,modelo="Sph"){
  ...
  alpha <- min(vari$gamma)
  beta <- max(vari$dist)
  m.xco2 <- gstat::fit.variogram(vari,gstat::vgm(0,modelo,beta,0))
  plot(vari,model=m.xco2, col=1,pl=F,pch=16)
  x<-df$longitude
  y<-df$latitude
  ...
  if(regiao == "sudente") pol_ <- pol_sudeste
